...
def request_issue_creation(exception_details):
    ...
    if question.lower().startswith("y"):
        ...
        if not is_newest:
            lib.formatter.error(
                "secureWall is not the newest version, in order to create an issue, please update secureWall"
            )
            return
        ...
        title = "secureWall Unhandled Exception ({})".format(identifier)

        python_version = "{}.{}{}".format(sys.version_info.major, sys.version_info.minor, sys.version_info.micro)
        issue_creation_template = {
            "title": title,
            "body": "secureWall version: `{}`\n"
                    "Running context: `{}`\n"
                    "Python version: `{}`\n"
                    "Traceback: \n```\n{}\n```\n"
...
def request_firewall_issue_creation(path):
    ...
    if question.lower().startswith("y"):
        ...
        if not is_newest:
            lib.formatter.error(
                "secureWall is currently not the newest version, please update to request a firewall script creation"
            )
            return
    ...
    lib.formatter.info("for further analysis the secureWAF fingerprint can be found in: '{}'".format(path))
