...
INSIDE_SAYING = '"\033[94mDevelopedBy secureNET?\033[0m\033[1m<|>v{}{}\033[1m"'.format(VERSION, VERSION_TYPE)
...
\t                                                    8888b   888 888            888     
\t                                                    88888b  888 888            888     
\t.d8888b   .d88b.   .d8888b 888  888 888d888 .d88b.  888Y88b 888 8888888        888     
\t88K      d8P  Y8b d88P"    888  888 888P"  d8P  Y8b 888 Y88b888 888            888     
\t"Y8888b. 88888888 888      888  888 888    88888888 888  Y88888 888            888     
\t     X88 Y8b.     Y88b.    Y88b 888 888    Y8b.     888   Y8888 888            888     
\t 88888P'  "Y8888   "Y8888P  "Y88888 888     "Y8888  888    Y888 8888888888     888     
...
def configure_request_headers(**kwargs):
    ...
    if proxy is not None:
        ...
        lib.formatter.warn(
            "it is highly advised to use a proxy when using secureWall. do so by passing the proxy flag "
...
def do_mine_for_whatwaf(proc_pid, start_time, start_it=True):
    """
    secureWall mining will no longer be done
    """
    lib.formatter.info("Skipping mining procedure")
    end__time=time.time()
    elapsed_time = end__time - start__time
    print(f"Scanning took {elapsed_time:.2f} seconds.")
    pass
...
def auto_update():
    ...
    try:
        lib.formatter.info("attempting to update secureWall")
        repo = git.Repo()
        current = repo.head.commit
        repo.remotes.origin.pull()
        new = repo.head.commit
        if current == new:
            lib.formatter.info("secureWall is the newest version")
        else:
            lib.formatter.success("successfully updated secureWall to the newest version")

    except:
        lib.formatter.error("unable to update secureWall, a new version is out attempt updating by typing `git pull`")
        pass
